        ...
        for p in hashes[i][v]:
            for q in hashes[j][v]:
                ...
                if (
                    len(
                        pd.DataFrame(s1)
                        .merge(pd.DataFrame(s2t + diff), how="inner", on=[0, 1, 2])
                        .index
                    )
                    >= 12
                ):
...
def map_hash(coords):
    ...
    for i in range(n):
        for j in range(n):
            if i != j:
                s[tuple(sorted(map(abs, coords[i, :] - coords[j, :])))] = (i, j)
    return s
...
def match(scanners, hashes):
    ...
    for i, j in combinations(range(len(hashes)), 2):
        if (m := len(set(hashes[i]) & set(hashes[j]))) >= math.comb(12, 2):
            yield i, j, next(iter(set(hashes[i]) & set(hashes[j])))
...
def solve(scanners):
    ...
    positions = {0: (0, 0, 0)}
    hashes = list(map(shash, scanners))
