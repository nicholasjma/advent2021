...
inst = list(map(Cube.from_str, lines))
x_values = sorted([x.xm for x in inst] + [x.xM + 1 for x in inst])
y_values = sorted([x.ym for x in inst] + [x.yM + 1 for x in inst])
z_values = sorted([x.zm for x in inst] + [x.zM + 1 for x in inst])
lit = set()


@cache
def get_x_index(x):
    return bisect_left(x_values, x)


@cache
def get_y_index(y):
    return bisect_left(y_values, y)


@cache
def get_z_index(z):
    return bisect_left(z_values, z)


def index_map(cube):
    xm = get_x_index(cube.xm)
    xM = get_x_index(cube.xM + 1)
    ym = get_y_index(cube.ym)
    yM = get_y_index(cube.yM + 1)
    zm = get_z_index(cube.zm)
    zM = get_z_index(cube.zM + 1)
    return Cube.from_coords(True, xm, xM - 1, ym, yM - 1, zm, zM - 1)


for cube in tqdm(inst):
    index = index_map(cube)
    mutate_func = lit.add if cube.state else lit.discard
    for x in index.xr:
        for y in index.yr:
            for z in index.zr:
                mutate_func((x, y, z))
...
sum(idx_vol(*k) for k in tqdm(lit))
```
