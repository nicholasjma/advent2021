...
def solve(smallest=False):
    ...
    while True:
        ...
        num = sum(ds[i] * 10 ** (13 - i) for i in range(14))
        if solutions:
            if smallest:
                solver.add(num < min(solutions))
            else:
                solver.add(num > max(solutions))
        z = 0
        ...
        if solver.check() == sat:
            solutions.append(solver.model().eval(num).as_long())
        else:
            break
    return min(solutions) if smallest else max(solutions)


print(solve())
print(solve(smallest=True))
```
