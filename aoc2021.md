...
class Target:
    def __init__(self, data):
        data = data.split("x=")[-1]
        self.xm, data, self.yM = data.split("..")
        self.xm, self.yM = map(int, (self.xm, self.yM))
        self.xM, self.ym = map(int, data.split(", y="))


def in_target(x, y, t):
    return t.xm <= x <= t.xM and t.ym <= y <= t.yM


def fire(vx, vy, target):
    x, y, maxh = 0, 0, 0
    while vy > 0 or y >= target.ym:
        x, y = x + vx, y + vy
        maxh = max(maxh, y)
...
def solution(target):
    outputs = [
        (h, vx, vy)
        for vx in range(int((2 * target.xm) ** 0.5), target.xM + 1)
        for vy in range(min(0, target.ym), target.xM + 1)
        if (h := fire(vx, vy, target)) > -math.inf
    ]
    return max(x[0] for x in outputs), len(outputs)


print(solution(Target(lines[0])))
```
